...
class DodoPaymentsAPI {
  ...
  async makeRequest(endpoint, options = {}) {
    ...
    if (!response.ok) {
      ...
      throw new Error(`DodoPayments API Error: ${response.status} - ${error.message || response.statusText}`);
    }
  ...
  async createSubscription(customerId, productId, trialDays = 5) {
    ...
    try {
      ...
      console.log('DodoPayments subscription body (top-level customer_id):', body2);
      return await this.makeRequest('/subscriptions', {
        method: 'POST',
